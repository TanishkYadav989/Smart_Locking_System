...
*Acoustic Wake-up & Monitoring*:- Uses a microphone to detect physical knocks on the door. The system "listens" in the background and only alerts the main processor when a significant acoustic event occurs (threshold reached), allowing the device to stay quiet and save power.

*Dynamic Visual Interface*:- Drives an OLED display to show system status & animations. It uses direct hardware memory transfers to keep animations smooth without slowing down the security logic using hardware timers.
...
*Intelligent Power Management*:- Automatically enters a low-power sleep state when idle. Crucial components and timing hardware remain active in the background to bring the system back "online" the instant a 3-knocks are detected on the door.
